...
2. Prompting players (or checking bots) if they have an identical card
3. Handling turn order interruption
4. Managing multiple simultaneous jump-in attempts
...
2. Create a `sounds/` directory in `card_games/uno/`
3. Add .wav files with the following names:
   - `card_play.wav`
   - `wild.wav`
   - `skip.wav`
   - `reverse.wav`
...
2. **7-0 Swapping**: Play 7 or 0 cards with the rule enabled
3. **Team Mode**: Start a 4-player game with team mode
4. **Animations**: Watch cards highlight when played in GUI
5. **Sound Effects**: Enable sounds and listen for audio feedback
...
5. Test with both console and GUI interfaces
