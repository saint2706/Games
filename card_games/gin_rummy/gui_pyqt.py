...
class GinRummyPyQtGUI(QMainWindow, BaseGUI):
    ...
    def build_layout(self) -> None:
        ...
        self.melds_label = QLabel(
            'Melds will appear here once available.', meld_group
        )
        self.melds_label.setWordWrap(True)
        meld_layout.addWidget(self.melds_label)

        self.deadwood_cards_label = QLabel(
            'Deadwood cards will be highlighted during play.', meld_group
        )
    ...
    def update_display(self) -> None:
        ...
        if self.game.discard_pile:
            ...
            self.top_discard_label.setText(
        ...
        self.stock_count_label.setText(
            f'Stock remaining: {len(self.game.deck.cards)}'
        )
    ...
    def on_take_initial_upcard(self) -> None:
        ...
        self.status_label.setText(
            f'You take {card}. Select a different card to discard or knock.'
        )
        self.update_display()
        self._update_controls()
    ...
    def on_draw_from_discard(self) -> None:
        ...
        self.phase = 'discard'
        self.status_label.setText(
            f'You take {card} from the discard pile. Choose a discard or knock.'
        )
        self.update_display()
        self._update_controls()
    ...
    def on_discard(self) -> None:
        ...
        self._flush_turn_log()
        self._prepare_next_action(
            override_message=f'You discard {discarded}. Waiting for opponent...'
        )
        self.update_display()
        self._update_controls()
        self._process_ai_turns()
        ...
        for card in sorted(self.players[0].hand, key=lambda c: (c.suit.value, c.value)):
            ...
            if card == self.selected_card:
                button.setStyleSheet(
                    f'background-color: {theme.colors.primary}; color: {theme.colors.button_active_fg};'
                    'font-weight: bold;'
                )
            else:
                button.setStyleSheet('')
    ...
    def _update_meld_display(self, analysis: HandAnalysis) -> None:
        ...
        if analysis.deadwood_cards:
            self.deadwood_cards_label.setText(
    ...
    def _update_player_badges(self) -> None:
        ...
        for idx, badge in enumerate(self.player_status_labels):
            ...
            badge.setText(text)
            badge.setStyleSheet(
                f'background-color: {bg}; color: {fg}; padding: 4px 8px; border-radius: 4px; font-weight: bold;'
            )
