...
to be run directly using the ``-m`` flag with Python. For example:
    ...
    python -m games_collection.games.card.uno --players 4 --bots 3

It simply imports and calls the ``main`` function from the ``uno`` module.
"""

# Keep imports minimal so ``python -m games_collection.games.card.uno`` starts quickly while
# delegating all heavy lifting to :func:`games_collection.games.card.uno.main`.
