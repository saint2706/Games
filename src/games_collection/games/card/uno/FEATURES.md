...
```bash
python -m games_collection.games.card.uno --gui pyqt --stacking --seven-zero --jump-in
```

### Team Mode with Stacking

```bash
python -m games_collection.games.card.uno --gui pyqt --team-mode --stacking --players 4
```
...
```bash
pip install pygame
python -m games_collection.games.card.uno --gui pyqt --seven-zero
...
1. Create a `sounds/` directory in `src/games_collection/games/card/uno/`
