...
Example Usage:
    python -m games_collection.games.card.poker.poker_hand_evaluator AS KD QH JC TC
...
if __name__ == "__main__":  # pragma: no cover - script entry point
    if len(sys.argv) <= 1:
        print(
            "Usage: python -m games_collection.games.card.poker.poker_hand_evaluator <card> [<card> ...]",
            file=sys.stderr,
        )
        print(
            "Example: python -m games_collection.games.card.poker.poker_hand_evaluator AS KD QH JC TC",
            file=sys.stderr,
        )
        sys.exit(1)
