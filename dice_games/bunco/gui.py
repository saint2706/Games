"""Tkinter-based graphical user interface for running Bunco tournaments.

This module provides a GUI application for simulating and visualizing a Bunco
tournament. It allows users to input player names, run the simulation, and view
the resulting bracket and scoreboard.

This GUI is built using the standard Python `tkinter` library and is designed
to be cross-platform. It gracefully handles the absence of `tkinter`, allowing
the rest of the application to function in a headless environment.

Classes:
    BuncoGUI: The main class for the Bunco tournament GUI.

Usage:
    To run the GUI, you typically instantiate `BuncoGUI` and pass it a `tk.Tk`
    root window. The application handles the rest.
"""
...
# Conditionally import tkinter to allow headless operation.
if TKINTER_AVAILABLE:
    import tkinter as tk
    from tkinter import messagebox, ttk
else:  # pragma: no cover - executed only when Tkinter is unavailable
    # Mock tkinter components if the library is not available.
    tk = None  # type: ignore
...
class BuncoGUI(BaseGUI):
    ...
    This class creates a window with controls for starting a tournament,
    displaying the bracket, and showing a summary of scores.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            tournament: An optional, pre-existing tournament to display.
            config: Configuration for the GUI window and animations.

        Raises:
            RuntimeError: If tkinter is not available in the environment.
        """
        ...
        # Set up GUI configuration or use defaults.
        ...
        # Widget references
        self.player_entry: Optional[ttk.Entry] = None
        self.bracket_text: Optional[tk.Text] = None
    ...
    def build_layout(self) -> None:
        ...
        assert TKINTER_AVAILABLE, "Tkinter must be available to build the layout."
        ...
        # --- Content Frame (Bracket and Scoreboard) ---
        ...
        self.bracket_text = tk.Text(bracket_frame, wrap="word", state="disabled", bg="#f0f0f0", relief="sunken")
        ...
        for col, title in headings.items():
            ...
            self.score_tree.column(col, anchor="center", width=100)
        self.score_tree.grid(row=0, column=0, sticky="nsew")

    def update_display(self) -> None:
        """Refresh the bracket and scoreboard widgets with current data."""
    ...
    def _run_tournament(self) -> None:
        """Parse player names from the entry field and simulate the tournament."""
        ...
        try:
            ...
            # Handle invalid player counts (not a power of two).
            if messagebox:
    ...
    def _update_bracket(self) -> None:
        """Render the textual representation of the tournament bracket."""
        ...
        if not self.tournament:
            ...
            # Iterate through rounds and matches to build the bracket text.
    ...
    def _update_scoreboard(self) -> None:
        """Update the scoreboard tree view with the latest player statistics."""
