"""This module demonstrates the practical application of the `common.cli_utils` package.

It implements a complete Number Guessing Game that leverages various CLI enhancement
features to create a rich, interactive, and user-friendly command-line experience.

This example showcases:
- Theming with `THEMES` for consistent color schemes.
- `ASCIIArt` for banners, boxes, and decorative elements.
- `RichText` for formatted and colored text output.
- `InteractiveMenu` for easy-to-navigate menus.
- `ProgressBar` and `Spinner` for indicating progress and loading states.
- `CommandHistory` for tracking user input and providing autocomplete suggestions.
"""

from __future__ import annotations

import random
import time

from games_collection.core.cli_utils import THEMES, ASCIIArt, Color, CommandHistory, InteractiveMenu, ProgressBar, RichText, Spinner


class NumberGuessingGame:
    """A Number Guessing Game that heavily utilizes the enhanced CLI utilities.

    This class encapsulates the game's logic, state, and presentation, serving as a
    comprehensive example of how to build a polished CLI application.
    """

    def __init__(self, min_number: int = 1, max_number: int = 100):
        """Initializes the game with default settings.

        Args:
            min_number: The minimum number in the guessing range.
            max_number: The maximum number in the guessing range.
        """
        self.min_number = min_number
        self.max_number = max_number
        self.secret_number = 0
        self.attempts = 0
        self.max_attempts = 10
        self.history = CommandHistory()
        self.theme = THEMES["ocean"]  # Using a predefined theme for styling.
        self.score = 0

    def show_welcome(self) -> None:
        """Displays the main welcome screen using ASCII art and rich text."""
        # `ASCIIArt.banner` creates a large, styled title.
        print(ASCIIArt.banner("NUMBER GUESSING GAME", self.theme.primary, width=60))
        print()
        # `RichText` provides methods for styled, semantic text.
        print(RichText.info("Guess the secret number to win!"))
        print(RichText.info(f"Range: {self.min_number} to {self.max_number}"))
        print(RichText.info(f"Attempts: {self.max_attempts}"))
        print()

    def show_loading(self) -> None:
        """Simulates a loading sequence using a spinner and a progress bar."""
        print(RichText.header("Preparing game...", level=2, theme=self.theme))
        print()

        # The `Spinner` provides feedback for an indeterminate wait time.
        spinner = Spinner(message="Generating secret number", theme=self.theme)
        spinner.start()
        for _ in range(5):
            time.sleep(0.1)
            spinner.tick()
        spinner.stop()

        # The `ProgressBar` shows progress for a task with a known duration.
        print("Loading game assets:")
        bar = ProgressBar(total=20, width=40, theme=self.theme)
        for i in range(21):
            bar.update(i)
            time.sleep(0.02)
        print()

    def show_main_menu(self) -> int:
        """Displays the main menu and returns the user's selection.

        Returns:
            The index of the selected menu option.
        """
        options = [
            "Start New Game",
            "Change Difficulty",
            "View High Score",
            "How to Play",
            "Quit",
        ]
        # `InteractiveMenu` handles display, navigation, and input for a list of options.
        menu = InteractiveMenu("Main Menu", options, theme=self.theme)
        return menu.display()

    def show_difficulty_menu(self) -> None:
        """Allows the player to select a new difficulty level."""
        print(RichText.header("Select Difficulty", level=2, theme=self.theme))
        print()

        options = [
            "Easy (1-50, 15 attempts)",
            "Medium (1-100, 10 attempts)",
            "Hard (1-200, 7 attempts)",
            "Expert (1-500, 5 attempts)",
        ]

        menu = InteractiveMenu("Difficulty", options, theme=self.theme)
        choice = menu.display()

        difficulty_settings = {
            0: (1, 50, 15),
            1: (1, 100, 10),
            2: (1, 200, 7),
            3: (1, 500, 5),
        }

        self.min_number, self.max_number, self.max_attempts = difficulty_settings[choice]
        print(RichText.success(f"Difficulty set to: {options[choice]}"))
        print()
        time.sleep(1)

    def show_how_to_play(self) -> None:
        """Displays the game's instructions in a styled box."""
        instructions = """How to Play:
1. The game picks a secret number.
2. You have a limited number of attempts to guess it.
3. After each guess, you'll get a hint (too high or too low).
4. Guess the number correctly within the attempt limit to win!

Special Commands:
  - 'hint': Get a hint about the number's range (costs 1 attempt).
  - 'quit': Exit the current game.
  - 'stats': See your current progress."""

        print()
        # `ASCIIArt.box` draws a decorative box around the text.
        print(ASCIIArt.box(instructions, self.theme.info, padding=2))
        print()
        input(RichText.info("Press Enter to return to the main menu..."))

    def start_new_game(self) -> None:
        """Sets up and begins a new round of the game."""
        self.secret_number = random.randint(self.min_number, self.max_number)
        self.attempts = 0
        print()
        print(RichText.header("New Game Started!", level=1, theme=self.theme))
        print(RichText.info(f"I'm thinking of a number between {self.min_number} and {self.max_number}."))
        print(RichText.info(f"You have {self.max_attempts} attempts to guess it!"))
        print()

        self.play_game()

    def play_game(self) -> None:
        """Contains the main loop for a single game round."""
        # Define a list of valid commands for autocomplete suggestions.
        commands = ["hint", "quit", "stats"]

        while self.attempts < self.max_attempts:
            remaining = self.max_attempts - self.attempts
            # Use `RichText.colorize` and `RichText.highlight` for emphasis.
            print(f"{RichText.colorize('Attempts remaining:', self.theme.text)} " f"{RichText.highlight(str(remaining), self.theme)}")

            try:
                # Style the input prompt.
                guess_input = input(f"{self.theme.accent}Your guess: {Color.RESET}").strip().lower()
            except (KeyboardInterrupt, EOFError):
                print()
                print(RichText.warning("Game interrupted. Returning to main menu."))
                return

            # Add every input to the command history.
            self.history.add(guess_input)

            # Handle special non-numeric commands.
            if guess_input == "quit":
                print(RichText.warning("Thanks for playing!"))
                return
            elif guess_input == "stats":
                self.show_stats()
                continue
            elif guess_input == "hint":
                self.give_hint()
                self.attempts += 1
                continue

            # Demonstrate autocomplete functionality.
            autocomplete = self.history.autocomplete(guess_input, commands)
            if autocomplete and autocomplete != guess_input:
                print(RichText.info(f"Autocomplete suggestion: {autocomplete}"))

            # Validate and process the user's guess.
            try:
                guess = int(guess_input)
            except ValueError:
                print(RichText.error("Invalid input. Please enter a number or a valid command."))
                continue

            self.attempts += 1

            # Check the guess against the secret number.
            if guess == self.secret_number:
                self.win_game()
                return
            elif guess < self.secret_number:
                print(RichText.warning("Too low! Try a higher number."))
            else:
                print(RichText.warning("Too high! Try a lower number."))
            print()

        self.lose_game()

    def give_hint(self) -> None:
        """Provides a hint to the player at the cost of one attempt."""
        range_size = self.max_number - self.min_number
        # Divide the number range into four quarters for the hint.
        hint_range = range_size // 4

        if self.secret_number <= self.min_number + hint_range:
            hint = f"The number is in the lower quarter ({self.min_number}-{self.min_number + hint_range})"
        elif self.secret_number <= self.min_number + 2 * hint_range:
            hint = f"The number is in the second quarter ({self.min_number + hint_range + 1}-{self.min_number + 2 * hint_range})"
        elif self.secret_number <= self.min_number + 3 * hint_range:
            hint = f"The number is in the third quarter ({self.min_number + 2 * hint_range + 1}-{self.min_number + 3 * hint_range})"
        else:
            hint = f"The number is in the upper quarter ({self.min_number + 3 * hint_range + 1}-{self.max_number})"

        print(RichText.info(f"Hint: {hint}"))

    def show_stats(self) -> None:
        """Displays the current game's statistics in a styled box."""
        stats = f"""Game Statistics:
  - Attempts used: {self.attempts}/{self.max_attempts}
  - Number Range:  {self.min_number}-{self.max_number}
  - Total Score:   {self.score}"""

        print()
        print(ASCIIArt.box(stats, self.theme.info, padding=1))
        print()

    def win_game(self) -> None:
        """Handles the logic for a winning game."""
        print()
        print(ASCIIArt.victory(self.theme.success))
        print()

        # Calculate and update the score.
        score = max(0, (self.max_attempts - self.attempts + 1) * 10)
        self.score += score

        print(RichText.success(f"Congratulations! You guessed the number in {self.attempts} attempts!"))
        print(RichText.success(f"You earned {score} points."))
        print(RichText.info(f"Your total score is now: {self.score}"))
        print()
        time.sleep(2)

    def lose_game(self) -> None:
        """Handles the logic for a losing game."""
        print()
        print(ASCIIArt.defeat(self.theme.error))
        print()
        print(RichText.error(f"Game Over! The secret number was {self.secret_number}."))
        print(RichText.info("Better luck next time!"))
        print()
        time.sleep(2)

    def run(self) -> None:
        """The main entry point and loop for the entire game application."""
        self.show_welcome()
        self.show_loading()

        while True:
            choice = self.show_main_menu()

            if choice == 0:  # Start New Game
                self.start_new_game()
            elif choice == 1:  # Change Difficulty
                self.show_difficulty_menu()
            elif choice == 2:  # View High Score
                print()
                print(RichText.highlight(f"Current High Score: {self.score} points", self.theme))
                print()
                input(RichText.info("Press Enter to continue..."))
            elif choice == 3:  # How to Play
                self.show_how_to_play()
            elif choice == 4:  # Quit
                print()
                print(ASCIIArt.banner("Thanks for Playing!", self.theme.success, width=60))
                print(RichText.success(f"Your Final Score: {self.score} points"))
                break


def main() -> None:
    """Main function to run the enhanced CLI game."""
    try:
        game = NumberGuessingGame()
        game.run()
    except KeyboardInterrupt:
        print()
        print(RichText.warning("\nGame interrupted by user. Exiting."))
    except Exception as e:
        print()
        print(RichText.error(f"An unexpected error occurred: {e}"))


if __name__ == "__main__":
    main()
